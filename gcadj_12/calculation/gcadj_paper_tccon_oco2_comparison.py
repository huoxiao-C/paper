...
while (oco2_begin_time!=oco2_end_time):
    ...
    filename = '/share/nas1_share1/huoxiao_share/CO2_data/Observation_data/satellite_on_gc_2x25grid/' \
               'OCO2_CO2_Litev9_ND_Observation_Error_Covariance/oco2_' \
               + oco2_begin_time.strftime('%Y%m%d%H%M%S') + '.nc'
...
all_oco2_xco2_array = np.zeros((50, 100000))
all_tccon_xco2_array = np.zeros((50, 100000))
all_prior_xco2_array = np.zeros((50, 100000))
all_posterior_xco2_array = np.zeros((50, 100000))
all_time_array = np.empty(shape=(50, 100000), dtype='<U30')
bias_array_hour_num_array = np.zeros(50)
...
for tccon_i in range(len(tccon_filenames)):
    ...
    oco2_xco2_array = np.zeros(100000)
    tccon_xco2_array = np.zeros(100000)
    prior_xco2_array = np.zeros(100000)
    posterior_xco2_array = np.zeros(100000)
    time_array = np.empty(shape=(100000), dtype='<U30')
    ...
    while(co2_begin_time_date!=co2_end_time_date):
        ...
        co2_begin_time_num2oco2 = date2num(co2_begin_time_date- datetime.timedelta(minutes=30), oco2_time_unit)
        co2_end_time_num2oco2 = date2num(co2_begin_time_date+datetime.timedelta(minutes=30), oco2_time_unit)
        ...
        oco2_lon_lt_tccon_lon = oco2_lon_ht_begin_time[np.where(oco2_lon_ht_begin_time>=(tccon_dic['lon']-lon_span))]
        oco2_lat_same_lat_lt = oco2_lat_ht_begin_time[np.where(oco2_lon_ht_begin_time>=(tccon_dic['lon']-lon_span))]
        oco2_xco2_same_lon_lt = oco2_xco2_ht_begin_time[np.where(oco2_lon_ht_begin_time >= (tccon_dic['lon']-lon_span))]

        oco2_lon_ht_tccon_lon = oco2_lon_lt_tccon_lon[oco2_lon_lt_tccon_lon<=(tccon_dic['lon']+lon_span)]
        oco2_lat_same_lat_ht = oco2_lat_same_lat_lt[oco2_lon_lt_tccon_lon <= (tccon_dic['lon']+lon_span)]
        oco2_xco2_same_lon_ht =  oco2_xco2_same_lon_lt[oco2_lon_lt_tccon_lon <= (tccon_dic['lon']+lon_span)]

        oco2_lat_lt_tccon_lat = oco2_lat_same_lat_ht[np.where(oco2_lat_same_lat_ht<=(tccon_dic['lat']+lat_span))]
        oco2_xco2_same_lat_lt = oco2_xco2_same_lon_ht[np.where(oco2_lat_same_lat_ht <=(tccon_dic['lat']+lat_span))]
        oco2_lon_same_lat_lt =oco2_lon_ht_tccon_lon[np.where(oco2_lat_same_lat_ht <= (tccon_dic['lat']+lat_span))]

        oco2_lat_ht_tccon_lat = oco2_lat_lt_tccon_lat[np.where(oco2_lat_lt_tccon_lat>=(tccon_dic['lat']-lat_span))]
        oco2_xco2_same_lat_ht = oco2_xco2_same_lat_lt[np.where(oco2_lat_lt_tccon_lat>=(tccon_dic['lat']-lat_span))]
        oco2_lon_same_lat_ht = oco2_lon_same_lat_lt[np.where(oco2_lat_lt_tccon_lat >= (tccon_dic['lat']-lat_span))]
        ...
        gc_lon_lt_tccon_lon = gcmet_nc_data.variables['lon'][
            np.where(gcmet_nc_data.variables['lon'][:] <= (tccon_dic['lon'] + lon_span))]
        gc_lon_ht_tccon_lon = gc_lon_lt_tccon_lon[gc_lon_lt_tccon_lon >= (tccon_dic['lon'] - lon_span)]

        gc_lat_lt_tccon_lat = gcmet_nc_data.variables['lat'][
            np.where(gcmet_nc_data.variables['lat'][:] <= (tccon_dic['lat'] + lat_span))]
        ...
        for i in range(gc_lon_num):
            for j in range(gc_lat_num):
                gc_co2_native_ori += (
                            gc_pri_co2[:, np.where(gc_lat_ht_tccon_lat[j] == gcmet_nc_data.variables['lat'][:])[0], \
                ...
                gc_pres += (gc_met_pres[:, np.where(gc_lat_ht_tccon_lat[j] == gcmet_nc_data.variables['lat'][:])[0], \
                            ...
                            / (gc_lon_num * gc_lat_num)).reshape(gc_level)
                gc_co2_native_ass += (
                            gc_pos_co2[:, np.where(gc_lat_ht_tccon_lat[j] == gcmet_nc_data.variables['lat'][:])[0],
                            np.where(gc_lon_ht_tccon_lon[i] == gcmet_nc_data.variables['lon'][:])[0]] \
                            / (gc_lon_num * gc_lat_num)).reshape(gc_level)
        ...
        prior_xco2_array[bias_array_hour_num] = gc_xco2_ori
        posterior_xco2_array[bias_array_hour_num]  = gc_xco2_ass
        oco2_xco2_array[bias_array_hour_num] = np.mean(oco2_xco2_same_lat_ht)
        tccon_xco2_array[bias_array_hour_num] = np.mean(np.array(tccon_dic['xco2']))
        time_array[bias_array_hour_num] = datetime.datetime.strftime(co2_begin_time_date, '%Y-%m-%d %H:%M:%S')
        co2_begin_time_date += datetime.timedelta(hours=1)
    ...
    all_prior_xco2_array[len(site_list), 0:bias_array_hour_num] = \
        prior_xco2_array[0:bias_array_hour_num]
    all_posterior_xco2_array[len(site_list), 0:bias_array_hour_num] = \
        posterior_xco2_array[0:bias_array_hour_num]
    all_oco2_xco2_array[len(site_list), 0:bias_array_hour_num] = \
        oco2_xco2_array[0:bias_array_hour_num]
    all_tccon_xco2_array[len(site_list), 0:bias_array_hour_num] = \
        tccon_xco2_array[0:bias_array_hour_num]
    all_time_array[len(site_list), 0:bias_array_hour_num] = \
        time_array[0:bias_array_hour_num]
    bias_array_hour_num_array[len(site_list)] = bias_array_hour_num
...
for i in range(len(site_list)):
    ...
    obs_num = int(bias_array_hour_num_array[i])
...
with pd.ExcelWriter('/home/huoxiao/paper_data/tccon_oco2_nd'+ '.xlsx',
                    mode='w') as writer:
    pddata.to_excel(writer, sheet_name='TCCON OCO2 DATA', engine='xlsxwriter')
