...
gc_pri_dir = root_dir+'huoxiao_share/GEOSChem_Simulation_test/'
gc_pos_dir = root_dir+'huoxiao_share/GEOSChem_Assimilation_Litev9_Nadir_Obsm_v2/'
obspack_dir = root_dir+'huoxiao_share/CO2_data/Observation_data/obspack_co2_1_CARBONTRACKER_CT2019_2020-01-16/data/nc/'
...
command_output = subprocess.check_output('ls '+obspack_dir+'*surface*_1_* | grep -v flask', shell=True)
obs_surface_filenames = files_string2list(command_output.decode())

obspack_dic_list = []
for obs_surf_filename in obs_surface_filenames:
    print(obs_surf_filename+' is processing')
    gc_ori_co2_list = []
    obspack_co2_list = []
    obspack_lon_list = []
    obspack_lat_list = []
    ...
    obs_surf_nc_data = Dataset(obs_surf_filename, 'r')
    #obs_flag
    obs_surf_time_large_scale = obs_surf_nc_data.variables['time'][np.where(obs_surf_nc_data.variables['obs_flag'][:]==1)]
    obs_surf_pre_large_scale = obs_surf_nc_data.variables['pressure'][np.where(obs_surf_nc_data.variables['obs_flag'][:] == 1)]
    obs_surf_co2_large_scale = obs_surf_nc_data.variables['value'][np.where(obs_surf_nc_data.variables['obs_flag'][:] == 1)]
    ...
    for dataith in range(obspack_time_value_array.shape[0]):
        ...
        lon_index, lat_index = get_ij(obs_surf_nc_data.variables['longitude'][dataith], \
                                      obs_surf_nc_data.variables['latitude'][dataith], delta_x, delta_y, lon_num,
                                      lat_num)
        ...
        obspack_lon_list.append(obs_surf_nc_data.variables['longitude'][dataith])
        obspack_lat_list.append(obs_surf_nc_data.variables['latitude'][dataith])
...
with pd.ExcelWriter('/home/huoxiao/paper_data/gc_surface''.xlsx', \
                    mode='w') as writer:
    pddata.to_excel(writer, sheet_name='OBSPACK SURFACE SITE DATA', engine='xlsxwriter')